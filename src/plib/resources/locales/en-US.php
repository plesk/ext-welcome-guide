<?php

$messages = [
    'index.config.confirm.popup.cancel'     => 'Cancel',
    'index.config.confirm.popup.content'    => 'Are you sure you want to replace existing configuration with the one from choosen preset? This action cannot be undone!',
    'index.config.confirm.popup.replace'    => 'Replace',
    'index.config.confirm.popup.title'      => 'Confirm configuration change',
    'index.config.label.json'               => 'Current JSON configuration:',
    'index.config.label.preset'             => 'Load configuration from a preset:',
    'index.config.message.success'          => 'JSON configuration saved successfully',
    'index.config.help.link'                => 'Help',
    'index.config.help.popup.title'         => 'Help Page',
    'index.config.help.popup.close'         => 'Close',
    'index.install.message.failure'         => 'Extension installation failed: %%error%%',
    'library.config.button.title.open'      => 'Open',
    'library.config.button.title.install'   => 'Install',
    'library.config.button.title.adddomain' => 'Add Domain',
    'welcomebox.tooltip.disable'            => 'Disable the Welcome Box completely',
    'welcomebox.dialog.disable.title'       => 'Disable the Welcome Box',
    'welcomebox.dialog.disable.content'     => 'You can disable the Welcome Box completely. Attention: Once disabled, you will not be able to enable it again!',
    'welcomebox.dialog.disable.button'      => 'Yes, disable the Welcome Box',
];
